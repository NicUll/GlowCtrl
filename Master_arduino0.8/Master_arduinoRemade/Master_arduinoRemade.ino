...
WireButton myWireButtons(1, 1500);
UTFT_Buttons myButtons(&myGLCD, &myTouch, &myWireButtons);
...
void WireButtonSetup() {
  strobbStor = myWireButtons.addWireButton(but5, 1, TOGGLE, CHASE_MASTER);
  strobbLiten = myWireButtons.addWireButton(but6, 2, TOGGLE, CHASE_MASTER);
  UVTak = myWireButtons.addWireButton(but10, 3, FULL_ON, SLIDE_ONLY);
  FX = myWireButtons.addWireButton(but11, 4, FLASH, SLIDE_ONLY);
  RGBSpot = myWireButtons.addWireButton(but8, 5, TOGGLE, CHASE_MASTER);
  miniLaser = myWireButtons.addWireButton(but7, 6, TOGGLE,  CHASE_MASTER);
  rinnande = myWireButtons.addWireButton(but4, 7, TOGGLE, SLIDE_ONLY);
  UVBar = myWireButtons.addWireButton(but9, 8, FULL_ON, SLIDE_ONLY);
...
void mainCheck() {
  ...
  Serial.println("mainCheck");
   if (myWireButtons.getWireButtonType(pressed) == CHASE_MASTER) { //Kolla om knappen är av typen chasemaster
      current_chase_group = myWireButtons.getChaseGroup(pressed);
      enableChaseButtons();
      prev_chase_group = current_chase_group;
    }


    else if (myWireButtons.getWireButtonType(pressed) == CHASE_MEMBER) {
          updateChaseButtons();
