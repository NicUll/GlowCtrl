...
UTFT_Buttons::UTFT_Buttons(UTFT *ptrUTFT, UTouch *ptrUTouch, Relay *ptrRelay)
{
	_UTFT = ptrUTFT;
	_UTouch = ptrUTouch;
	_Relay = ptrRelay;
...
void UTFT_Buttons::deleteButton(int buttonID)
{
	if (!(buttons[buttonID].flags & BUTTON_UNUSED)){
		...
		if (_Relay->isRelayActive(buttonID) == true){
			_Relay->deleteRelay(buttonID);
		}
	}
}
...
void UTFT_Buttons::deleteAllButtons()
{
	for (int i=0;i<MAX_BUTTONS;i++)
	{
		...
		buttons[i].label="";
		if (_Relay->isRelayActive(i) == true){
			_Relay->deleteRelay(i);
		}
	}
}
...
int UTFT_Buttons::checkButtons(boolean phys, int buttonID)
{
    ...
    if (_UTouch->dataAvailable() == true)
    {
		...
		if (result != -1)
		{
			...
			if (_Relay->isRelayActive(result) == true){

				relayFunction = _Relay->relayFunction(result);
				_Relay->relaySwitch(result);
				...
				if (_Relay->isRelayTurnedOn(result) == true && relayFunction == TOGGLE){

					timer = millis();
					word switchColor = _Relay->getAltColor(result);
					while (_UTouch->dataAvailable() == true) {
						if ((millis() - timer) > _Relay->toggleTime){
							markButton(result, switchColor, true);
							while (_UTouch->dataAvailable() == true){
							}
						}
					}
					switchColor = _Relay->getStaColor(result);
...
{
	int relayFunction = _Relay->relayFunction(buttonID);
	if (relayFunction != FLASH && _Relay->isRelayTurnedOn(buttonID) == true){
		_UTFT->setColor(VGA_LIME);
		_UTFT->fillCircle(buttons[buttonID].pos_x + buttons[buttonID].width - 9, buttons[buttonID].pos_y + 9, 6);
	}

	if (relayFunction != FLASH && _Relay->isRelayTurnedOn(buttonID) == false){
