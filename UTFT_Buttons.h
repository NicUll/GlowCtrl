...
#define MAX_BUTTONS	48	// Maximum number of buttons available at one time
...
{
	public:
		UTFT_Buttons(UTFT *ptrUTFT, UTouch *ptrUTouch, WireButton *ptrWireButton);
		...
		int		checkButtons(boolean phys=false, int buttonID=-1);
		void	setTextFont(uint8_t* font);
		void	setSymbolFont(uint8_t* font);
		void	setButtonColors(word atxt, word iatxt, word brd, word brdhi, word back);
		void	markButton(int buttonID, word switchColor, boolean fade=false);
		void	enableButtonDraw(int buttonID);
		void	disableButtonDraw(int buttonID);
		void	drawButtonStatus(int buttonID);
		void	drawAllButtonStatus();

	protected:
		UTFT		*_UTFT;
		UTouch		*_UTouch;
		WireButton		*_WireButton;
